...
public:
    ...
    TreeNode* contruct(vector<int>&pre, vector<int>&vin, int l1, int r1, int l2, int r2)
    {
        ...
        // 中序遍历的根节点位置在中间 rt，在 rt 左边的肯定是node的左子树的中序子数组vin[left...rt-1],
        // rt右边的肯定是node的右子树的中序子数组vin[rt+1...right]
        // 先序遍历的第二个位置到rt，即pre[left+1...rt]，为node左子树的先序子数组，
        // 剩下rt右边的 pre[rt+1...right]就是node的右子树的先序子数组
        if(left_tree_len > 0)
        {
            // 根节点 rt 的左子树，递归处理
